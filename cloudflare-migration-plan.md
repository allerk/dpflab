...
- [ ] Сидинг: перегнать `seed.ts` в `seed.sql` (или одноразовый скрипт через wrangler), применить `wrangler d1 execute dpflab --file=seed.sql`. **Только при первом наполнении.**
...
1. **Сидинг прод-данных**: наполняем D1 актуальным контентом сразу из `seed.sql` или ты заводишь контент через админку после деплоя?
